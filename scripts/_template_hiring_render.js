/**
 * TEMPLATE: Hiring-report PDF renderer
 *
 * Fork this for the next hiring-guide build. Replace SLUG and CANDIDATE_NAME
...
const REPORT_HTML = path.join(__dirname, '..', '..', '_reports', `${SLUG}_hiring_guide.html`);
const OUTPUT_PDF  = path.join(__dirname, '..', '..', '_reports', `${SLUG}_hiring_guide_${PDF_VERSION}.pdf`);
const HEADER_TEXT = `Hale Global Success Diagnostics \u00b7 Excellence Standards Hiring Manager Report \u00b7 ${CANDIDATE_NAME}`;
...
const PRINT_CSS = JSON.parse(fs.readFileSync(PRINT_CSS_PATH, 'utf8')).hiring.join('\n');
